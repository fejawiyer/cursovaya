...
class MPCAnimation:
    def __init__(self, anim_int, x_size, y_size, mpc, npz_dir,
                 output_file=None, progress_bar=None, zoning=False, repeat=True):
        ...
        self.c_list = np.load(str(npz_dir))['res'].transpose(0, 2, 1)
...
class DefaultAnimation:
    def __init__(self, anim_int, x_size, y_size, npz_dir, output_file=None,
                 progress_bar=None, zoning=False, update_conc=False, repeat=True):
        if output_file is not None:
