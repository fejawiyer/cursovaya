...
from PyQt5.QtWidgets import (QWidget, QLabel, QApplication, QMenuBar, QDesktopWidget, QAction, QDialog, QGridLayout,
                             ...
                             QMessageBox)
from core.Conditions import NewConditions
...
from core.Wind import WindRulesDialog
...
class App(QWidget):
    def __init__(self):
        super().__init__()
        try:
            self.load_stylesheet(CONFIG_DIR / "styles" / "style_dark.qss")
        except Exception as e:
            QMessageBox.critical(self, "Ошибка", "Файл style_dark.qss не найден.")
            logging.error(f"{e}")
        ...
        self.grid_layout.addWidget(self.iterate_button, 8, 2)
        ...
        self.grid_layout.addWidget(self.ok_button, 9, 2)
        ...
        self.grid_layout.addWidget(self.save_list, 10, 2)
        ...
        self.grid_layout.addWidget(self.save_button, 11, 2)

        self.grid_layout.addWidget(self.progress_bar, 12, 2)
        ...
        self.x_size = self.y_size = self.x_step = self.y_step = self.Dx = self.Dy = self.u = self.v = self.t \
            = self.t_step = self.anim_int = self.freq = self.x = self.y = self.c = self.repeat_freq = self.update_conc \
            = self.sources = self.check_stable = self.check_cfl = self.conditions = self.debug = None

        self.npz_exists = os.path.isfile(DATA_DIR / "model.npz")
    ...
    def newWindMatrixDialog(self):
        pass

    def light_theme(self):
        self.load_stylesheet(CONFIG_DIR / "styles" / "style.qss")
    ...
    def newWindDialog(self):
        try:
            dialog = WindRulesDialog(parent=self, wind_path=CONFIG_DIR / "wind.json")
            if dialog.exec_() == QDialog.Accepted:
                rules = dialog.get_rules()
                with open(CONFIG_DIR / "wind.json", 'w') as f:
    ...
    def check_logs(self):
        try:
            logsDialog = LogViewerDialog(parent=self, logs_dir=PROJECT_DIR / 'app.log')
            logsDialog.exec_()
        except Exception as e:
            print(f"{e}")

    def pdk_table_dialog(self):
        dialog = SubstancesDialog(CONFIG_DIR / "substances.json", parent=self)
        if dialog.exec_() == QDialog.Accepted:
    ...
    def load_substances(self):
        try:
            with open(CONFIG_DIR / 'substances.json', 'r', encoding='utf-8') as file:
                data = json.load(file)
    ...
    def get_params(self):
        try:
            ...
            if tmp == "Дирихле":
                ...
                self.conditions = "WTF"
            c_start_file = CONFIG_DIR / "parameters.json"
            with open(c_start_file, "r") as config:
            ...
            wind_file = CONFIG_DIR / "wind.json"
            with open(wind_file, "r") as config:
                config_data = json.load(config)
    ...
    def createNewConditions(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            try:
                ...
                with open(CONFIG_DIR / 'parameters.json', 'w', encoding='utf-8') as f:
    ...
    def saveFile(self):
        data = {
            ...
            "initial_conditions": CONFIG_DIR / "parameters.json",
    ...
    def iterate(self):
        ...
        try:
            model = Model(config_dir=CONFIG_DIR,
            ...
            model.iterate()
            res = model.c_list
            np.savez(DATA_DIR / "model.npz", res=res)

        except Exception as e:
            logging.error(f"{e}")

    def show_it(self):
        self.iterate() if self.npz_exists is False else None
        self.get_params()
        try:
            zoning = True if self.interpolation_method.currentText() == "Билинейная" else False
            if self.mpc_use:
                plot = MPCAnimation(self.anim_int, self.x_size, self.y_size,
                                    self.get_current_pdk(), npz_dir=DATA_DIR / "model.npz", zoning=zoning)
                plot.draw_or_save()
            else:
                plot = DefaultAnimation(self.anim_int, self.x_size, self.y_size,
                                        update_conc=self.update_conc, zoning=zoning, npz_dir=DATA_DIR / "model.npz")
                plot.draw_or_save()
        except Exception as e:
            logging.error(f"{e}")
    ...
    def save_it(self):
        self.get_params()
        output = DATA_DIR / "animation" / ("animation" + self.save_list.currentText())
        try:
            zoning = True if self.interpolation_method.currentText() == "Билинейная интерполяция" else False
            if self.mpc_use:
                saver = MPCAnimation(anim_int=self.anim_int, x_size=self.x_size, y_size=self.y_size,
                                     mpc=self.get_current_pdk(), output_file=output, zoning=zoning,
                                     progress_bar=self.progress_bar, npz_dir=DATA_DIR / "model.npz")
                saver.draw_or_save()
            else:
                saver = DefaultAnimation(anim_int=self.anim_int, x_size=self.x_size, y_size=self.y_size,
                                         output_file=output, zoning=zoning, progress_bar=self.progress_bar,
                                         npz_dir=DATA_DIR / "model.npz")
                saver.draw_or_save()
        except Exception as e:
            logging.error(f"{e}")
...
if __name__ == '__main__':
    logging.basicConfig(filename=PROJECT_DIR / 'app.log',
                        level=logging.INFO,
